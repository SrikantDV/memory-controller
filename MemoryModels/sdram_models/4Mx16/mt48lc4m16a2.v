    ...
    always @ (posedge Sys_clk) begin
        ...
        if (Aref_enable == 1'b1) begin
            ...
            if ($time - RC_chk < tRC) begin
                $display ("at time %t ERROR: tRC violation during Auto Refresh, RC_chk=%d, tRC=%d", $time, RC_chk, tRC);
            end
        ...
        if (Active_enable == 1'b1) begin
			...
			$fdisplay(FH,"%t : PRECHARING BANK01 : ENTRY - 1", $time);
                {Act_b1, Pc_b1} = 2'b10;
			...
			$fdisplay(FH,"%t : PRECHARING ERROR : ENTRY - 2", $time);
